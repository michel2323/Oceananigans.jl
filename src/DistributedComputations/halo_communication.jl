...
function mpi_isend(buffer, rank_to_send_to, send_tag, comm)
  if architecture(buffer) isa GPU
...
function mpi_irecv!(dest, source, tag, comm)
  if architecture(dest) isa GPU
    host_buf = Array(dest)
    req = MPI.Irecv!(host_buf, source, tag, comm)
    return DeviceIrecvRequest{eltype(dest)}(req, dest, host_buf)
  else
...
#   mapping = Dict{MPI.Request, DeviceIrecvRequest}()
  dev_bufs = Vector{DeviceIrecvRequest}()
  for r in requests
    if r isa DeviceIrecvRequest
  ...
  if MPI.Comm_rank(MPI.COMM_WORLD) == 0
    @debug "Waiting for $(length(host_reqs)) host requests and $(length(dev_bufs)) device requests"
  end
  MPI.Waitall(host_reqs)
#   for (host_req, dev_req) in mapping
  for dev_buf in dev_bufs
    # copyto!(dev_req.device_array, dev_req.host_buffer)
    copyto!(dev_buf.device_array, dev_buf.host_buffer)
  end
  return nothing
end
